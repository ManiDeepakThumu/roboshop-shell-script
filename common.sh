ID=$(id -u)
if [ $ID -ne 0 ]; then
 echo You should run this script as root user or with sudo prvileges.
 exit 1
fi
...
SYSTEMD_SETUP() {
  ...
  sed -i -e 's/REDIS_ENDPOINT/redis.roboshop.internal/' -e 's/MONGO_ENDPOINT/mogodb.roboshop.internal/' -e 's/CATALOGUE_ENDPOINT/catalogue.roboshop.internal/' -e 's/MONGO_DNSNAME/mongodb.roboshop.internal/' -e 's/CARTENDPOINT/cart.roboshop.internal/' -e 's/DBHOST/mysql.roboshop.internal/' -e 's/CARTHOST/cart.roboshop.internal/' -e 's/USERHOST/user.roboshop.internal/' -e 's/AMQPHOST/rabbitmq.roboshop.internal/' -e 's/DISPATCH/dispatch.roboshop.internal/' /home/roboshop/${COMPONENT}/systemd.service
  StatusCheck $?
